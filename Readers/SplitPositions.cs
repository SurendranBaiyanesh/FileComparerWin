...
public static class SplitPositions
{
	...
	public static int[] Parse(string configured)
	{
		...
		[
			.. configured.Split([';', ',', ' ', '\t'], StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries)
			             .Select(part => int.TryParse(part, System.Globalization.NumberStyles.Integer, System.Globalization.CultureInfo.InvariantCulture, out int value) ? value : -1)
			             .Where(value => value >= 0)
			             .Distinct()
			             .Order()
		];
	...
	public static string Strip(string marked, char marker)
	{
		return marked.Replace(marker.ToString(), string.Empty);
	}
