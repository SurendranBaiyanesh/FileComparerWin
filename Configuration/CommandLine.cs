...
public static class CommandLine
{
	...
	public static string GetConfigPath(string[] args, string defaultPath)
	{
		...
		{
			if(Matches(args[i], "--config")) return args[i + 1];
		}
	...
	{
		return args.Any(a => Matches(a, "--run", "--compare"));
	}
	...
	{
		return value is null || value.StartsWith('-') || !bool.TryParse(value, out bool parsed) || parsed;
	}
	...
	{
		return decimal.TryParse(value, NumberStyles.Float, CultureInfo.InvariantCulture, out decimal parsed) ? parsed : fallback;
	}
	#endregion
