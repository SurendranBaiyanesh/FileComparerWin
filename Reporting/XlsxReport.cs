...
public static class XlsxReport
{
	...
	private static Sheet MatchingValues(ComparisonResult result)
	{
		...
		{
			if(sheet.IsFull) return sheet;

			AddValue(sheet, result, row.DisplayKey, column, result.Input.GetValue(row.InputRow, column), result.Output.GetValue(row.OutputRow, column), result.Input, row.InputRow);
		}
	...
	private static Sheet AdditionalInOutput(ComparisonResult result)
	{
		...
		foreach(string column in result.ComparedColumns)
		{
			...
			AddValue(sheet, result, row.DisplayKey, column, string.Empty, result.Output.GetValue(row.Row, column), result.Output, row.Row);
		}
	...
	private static Sheet MissingFromOutput(ComparisonResult result)
	{
		...
		foreach(string column in result.ComparedColumns)
		{
			...
			AddValue(sheet, result, row.DisplayKey, column, result.Input.GetValue(row.Row, column), string.Empty, result.Input, row.Row);
		}
	...
	private static Sheet SimilarMatches(ComparisonResult result, ComparisonOptions options)
	{
		...
		foreach(ValueDifference value in match.Values)
		{
			...
			AddValue(sheet, result, match.DisplayKey, value.Column, value.InputValue, value.OutputValue, result.Input, match.InputRow);
		}

		// With the option off there is no range to name, and "(±0) (0)" on the tab reads as a fault.
		NameWithCount(sheet, options.SimilarMatch && options.SimilarMatchRange > 0 ? $"Similar matches (±{Describe(options.SimilarMatchRange)})" : "Similar matches", "Similar matches");
	...
	private static Sheet DifferenceValues(ComparisonResult result)
	{
		...
		foreach(ValueDifference difference in mismatch.Differences)
		{
			...
			AddValue(sheet, result, mismatch.DisplayKey, difference.Column, difference.InputValue, difference.OutputValue, result.Input, mismatch.InputRow);
		}
	...
	private static void AddValue(Sheet sheet, ComparisonResult result, string key, string column, string inputValue, string outputValue, DataTable table, DataRow row)
	{
		sheet.AddText(key, inputValue, outputValue, Difference(inputValue, outputValue), OtherValues(result, table, row, column));
	}
	...
	{
		if(TryReadNumber(inputValue, out decimal left) && TryReadNumber(outputValue, out decimal right)) return Describe(right - left);
	...
	{
		return string.Equals(TextKey.Canonical(left), TextKey.Canonical(right), StringComparison.OrdinalIgnoreCase);
	}

	private static string SimilarHeading(ComparisonOptions options)
	{
		return $"Similar matches ({PlusMinus(options)}) in the output";
	}

	private static string PlusMinus(ComparisonOptions options)
	{
		return $"+/-{Describe(options.SimilarMatchRange)}";
	}
	...
	{
		return cell.Amount is { } number ? number.ToString(CultureInfo.InvariantCulture).Length : cell.Value?.Length ?? 0;
	}
	...
	{
		return value.Any(IsForbidden) ? new string([.. value.Where(c => !IsForbidden(c))]) : value;
	}
	...
	private sealed class Sheet
	{
		...
		public int DataRowCount => this.Rows.Count - this.HeaderRows.Count;
		...
		{
			this.Rows.Add([Cell.Text(label)]);
		}

		public void Add(string label, string value)
		{
			this.Rows.Add([Cell.Text(label), Cell.Text(value)]);
		}

		public void AddText(params string[] values)
		{
			this.Rows.Add([.. values.Select(Cell.Text)]);
		}
		...
		{
			this.HeaderRows.Add(this.Rows.Count);
			AddText(values);
		}
	...
	private readonly record struct Cell(string? Value, decimal? Amount)
	{
		...
		public static Cell Number(decimal value)
		{
			return new Cell(null, value);
		}

		public bool IsEmpty => this.Amount is null && string.IsNullOrEmpty(this.Value);
	}
	#endregion
}