...
public sealed class DataTable
{
	...
	#region Constructor
	public DataTable(string sourcePath, string formatName, IReadOnlyList<string> columns, IReadOnlyList<DataRow> rows, string? delimiter = null, bool generatedColumnNames = false)
	{
		...
		this.HasGeneratedColumnNames = generatedColumnNames;
		...
		for(int i = 0; i < columns.Count; i++) _columnIndex.TryAdd(TextKey.Canonical(columns[i]), i);
	}
	#endregion
	...
	{
		return _columnIndex.ContainsKey(TextKey.Canonical(name));
	}

	public string GetValue(DataRow row, string column)
	{
		if(!_columnIndex.TryGetValue(TextKey.Canonical(column), out int index)) return string.Empty;
	...
	{
		return _columnIndex.TryGetValue(TextKey.Canonical(name), out int index) ? this.Columns[index] : name;
	}
	#endregion
}
