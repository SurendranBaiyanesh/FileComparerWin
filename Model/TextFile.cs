...
public static class TextFile
{
    ...
    public static TextContent Read(string path, string requestedEncoding = "")
    {
        ...
        {
            if (IsWindows1252(requestedEncoding))
        ...
        if (declared is not null)
            return new TextContent(Decode(bytes, declared), Describe(declared));
    ...
    private static string DecodeWindows1252(byte[] bytes)
    {
        ...
        for (int i = 0; i < bytes.Length; i++)
            characters[i] = bytes[i] is >= 0x80 and <= 0x9F ? Windows1252Upper[bytes[i] - 0x80] : (char)bytes[i];

        return new string(characters);
    }

    private static bool IsWindows1252(string name) =>
        name.Trim().ToLowerInvariant() is "windows-1252" or "windows1252" or "cp1252" or "1252" or "ansi";

    private static Encoding Resolve(string name) => name.Trim().ToLowerInvariant() switch
    {
        ...
        _ => throw new NotSupportedException(
            $"Unknown encoding '{name}'. Supported: utf-8, utf-16, utf-16be, utf-32, ascii, latin1, windows-1252.")
    };
